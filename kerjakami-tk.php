<?php include 'layout/header.php'; ?>
</head>

<body>
    <?php include 'layout/navbar.php' ?>
    <div class="pt-16">
        <div class="">
            <img class="object-cover w-full h-96 relative z-10" src="https://source.unsplash.com/collection/190728/3000x900" alt="image" />
            <div class="bg-black -mt-24 relative z-20 w-96 mb-4 mx-auto rounded-md bg-opacity-60">
                <p class="p-3 font-semibold text-lg text-white text-center">Program Kerja Kami pada<br>Taman Kanak-Kanak</p>
            </div>
        </div>
        <div class="mx-40 pt-10 pb-8 px-4 bg-gray-100">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-0">
                <div class="grid col-span-1 p-6">
                    <a href="tk-1.php">
                        <button class="hover:border-red-500 border-2 rounded-md border-gray-100">
                            <div class="col-span-1 p-3 bg-white shadow-md rounded-md">
                                <img class="object-cover w-screen h-52" src="public/tk/tk-1-b.jpg">
                                <p class=" pl-2 mt-1 py-2 text-base text-left font-semibold">TK Sekolah jauh desa Pedawa, Singaraja, Kabupaten Buleleng</p>
                                <div class="mr-auto w-48  py-2 px-4 bg-red-500 hover:bg-red-700 text-white text-xs md:text-sm lg:text-sm font-bold rounded-md">
                                    Baca Selengkapnya >
                                </div>
                            </div>
                        </button>
                    </a>
                </div>
                <div class="grid col-span-1 p-6">
                    <a href="#">
                        <button class="hover:border-red-500 border-2 rounded-md border-gray-100">
                            <div class="col-span-1 p-3 bg-white shadow-md rounded-md">
                                <img class="object-cover w-screen h-52" src="public/sd/sdn7gianyar_rakbuku.jpg">
                                <p class=" pl-2 mt-1 py-2 text-base text-left font-semibold">Rak Buku Gianyar</p>
                                <div class="mr-auto w-48  py-2 px-4 bg-red-500 hover:bg-red-700 text-white text-xs md:text-sm lg:text-sm font-bold rounded-md">
                                    Baca Selengkapnya >
                                </div>
                            </div>
                        </button>
                    </a>
                </div>
                <div class="grid col-span-1 p-6">
                    <a href="#">
                        <button class="hover:border-red-500 border-2 rounded-md border-gray-100">
                            <div class="col-span-1 p-3 bg-white shadow-md rounded-md">
                                <img class="object-cover w-screen h-52" src="public/sd/sdn7gianyar_rakbuku.jpg">
                                <p class=" pl-2 mt-1 py-2 text-base text-left font-semibold">Rak Buku Gianyar</p>
                                <div class="mr-auto w-48  py-2 px-4 bg-red-500 hover:bg-red-700 text-white text-xs md:text-sm lg:text-sm font-bold rounded-md">
                                    Baca Selengkapnya >
                                </div>
                            </div>
                        </button>
                    </a>
                </div>
                
            </div>
        </div>
    </div>


    <!--Footer -->
    <?php include 'layout/footer.php'; ?>